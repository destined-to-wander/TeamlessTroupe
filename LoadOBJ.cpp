...
)
{
	...
	while (!fileStream.eof()) {
		...
		if (strncmp("v ", buf, 2) == 0) { 
			...
			glm::vec3 vertex;
			sscanf_s((buf + 2), "%f%f%f", &vertex.x, &vertex.y, &vertex.z); //1st, skips 2 lines, the 'v ' part. Then it takes up to 3 floats
			...
			glm::vec2 texCoord;
			sscanf_s((buf + 2), "%f%f", &texCoord.s, &texCoord.t); //1st, skips 2 lines, the 'v ' part. Then it takes up to 2 floats
			...
			glm::vec3 normal;
			sscanf_s((buf + 2), "%f%f%f", &normal.x, &normal.y, &normal.z); //1st, skips 2 lines, the 'v ' part. Then it takes up to 3 floats
